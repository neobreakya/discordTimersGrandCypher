  ...
  new SlashCommandBuilder()
    ...
    .addChannelOption((option) =>
      option
        .setName('channel')
        .setDescription(
          'Optional: Post this event in a specific channel (overrides server default)'
        )
        .setRequired(false)
    ),
...
async function updateAllEvents() {
  for (const [guildId, defaultChannelId] of guildChannels) {
    const events = guildEvents.get(guildId) || [];

    for (const event of events) {
      try {
        // Use channel override if specified, otherwise use guild default
        const channelId = event.channelOverride || defaultChannelId;
        ...
        console.error(
          `Error updating ${event.name} in guild ${guildId}:`,
          error.message
        );
      }
    }
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === 'setchannel') {
    ...
    const event = {
      ...
      duration,
      days,
      channelOverride: channelOverride?.id || null,
    };
    ...
    await interaction.reply({
      ...
      } event **${name}**${timeInfo}${channelInfo}\nDuration: ${formatDuration(
        duration
    ...
    if (eventMessages.has(messageKey)) {
      ...
      const channelId = event?.channelOverride || guildChannels.get(guildId);

      if (channelId) {
        try {
          const channel = await client.channels.fetch(channelId);
          const msg = await channel.messages.fetch(
            eventMessages.get(messageKey)
          );
          await msg.delete();
        } catch (error) {
    ...
    const defaultChannelId = guildChannels.get(guildId);
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        events
          .map((e) => {
            let channelInfo = '';
            if (e.channelOverride) {
              channelInfo = ` | 📍 <#${e.channelOverride}>`;
            }
            return `**${e.name}**\n⏰ ${e.startTime} | ⏳ ${formatDuration(
              e.duration
            )} | 📅 ${formatDays(e.days)}${channelInfo}`;
          })
          .join('\n\n')
      )
      .setFooter({
        text: `Default Channel: ${
          defaultChannelId ? `<#${defaultChannelId}>` : 'Not set'
        }`,
      });
